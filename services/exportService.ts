...
export const generateGradingRubric = (assignment: Assignment): object => {
  ...
  const isReader = assignment.assignmentKind === 'reader';
  ...
  assignment.problems.forEach((prob, pIndex) => {
    prob.subsections.forEach((sub, sIndex) => {
      ...
      rubrics[subsectionId] = {
        ...
        // NO grading_type ON A READER ASSIGNMENT (decided 2026-09-24). Nothing
        // grades it, so `ai_handwritten` and `human_handwritten` would both be
        // false; `assignment_kind: "reader"` already says who grades it, which
        // is nobody. An absent field is an absence rather than a false
        // statement — the rule `answer_modality` follows. The authored
        // `[handwritten]` / `[handwritten:human]` suffix is therefore IGNORED on
        // a reader assignment, not refused: either tag gives the same export.
        // The spread keeps the key where it always was on a conventional part,
        // so conventional rubrics are byte-identical.
        ...(isReader ? {} : {
        ...
        // On a reader assignment the tag is ignored here too, or the two tags
...
/**
 * The same four files on a READER export, described as what they hold
 * (2026-09-24). Nothing grades a reader assignment, so none of them is an
 * answer key and the conventional wording sent the instructor looking for one
 * that was never written — and a missing key reads as a failed export. They
 * are still instructor-only: they carry the questions ahead of release and any
 * note the author typed.
 */
const READER_INSTRUCTOR_FILES: Array<[suffix: string, what: string]> = [
  ['_grader_document.html',   'the questions and any notes you wrote, for you'],
  ['_grading_rubric.json',    'each part and its region, for the reading side'],
  ['_authoring_backup.json',  'the complete assignment'],
...
const buildDistributionNotice = (
  ...
  // The branch on kind. Every reader line below is the conventional line's
  // counterpart; the conventional text is unchanged.
  const reader = assignmentKind === 'reader';
  const answerRows = (reader ? READER_INSTRUCTOR_FILES : ANSWER_BEARING)
    .map(([suffix, what]) => {
  ...
  return [
    ...
    '',
    ...(reader ? [
      'This is a reader assignment. Nothing grades it, so this export has no',
      'marking rubric and no answer key, and none is missing.',
    ] : [
    ...
    reader
      ? 'Keep everything else. It is your backup.'
