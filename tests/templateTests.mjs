...
// region-height fix landed, and the break silently stopped happening.
{
  ...
  // was entirely absent otherwise. It was added on 2026-09-06 for a real
  // reason: the tool had claimed the ground and left it empty, and a student
  // printed sixteen sheets, did the work properly, and was holding paper with
  // no stated next step.
  //
  // The section and the field are removed. Students are told how to hand work
  // in when they open the assignment in the Submission app, which is where they
  // actually meet the question, and no real assignment ever set an address —
  // so the sheet those tests described was one nobody ever printed.
  //
  // These two checks are what is left, and they are the ones worth keeping: the
  // section really is gone from the page, and its absence moved no geometry.
  check('submission section: page 1 no longer carries it, whatever the assignment says', async () => {
    const withStaleField = instr({});
    // Set through a cast: the field is off `Assignment` now, and the point is
    // that a stored project still carrying one changes nothing on paper.
    withStaleField.submissionAddress = 'submit.example.edu/demo';
    const g = await gen.generateTemplate(withStaleField);
    const page1 = g.layout.instructionsPage;
    const text = JSON.stringify(page1);
    assert(!/When you have finished writing/i.test(text),
      'the submission heading is still printed on page 1');
    assert(!/submit\.example\.edu/i.test(text),
      'a stale submission address still reaches the printed page');
    assert(!/Photograph each page/i.test(text),
      'the submission steps are still printed on page 1');
  });

  check('submission section: removing it moved no geometry', async () => {
    const plain = await gen.generateTemplate(instr({}));
    const stale = instr({});
    stale.submissionAddress = 'submit.example.edu/demo';
    const withField = await gen.generateTemplate(stale);
    assertEqual(withField.layoutId, plain.layoutId,
      'a stale submission address moved the layout id');
    assertEqual(withField.pageCount, plain.pageCount,
      'a stale submission address changed the page count');
    assertEqual(withField.layout.instructionsPage.overflowMm, 0,
      'the instructions page overflows without the section');
  });

  check('submission section: the self-test no longer warns about a missing address', async () => {
    // It used to warn on every assignment that had no address, which was all of
    // them, and told the instructor to set a field that no longer exists.
    const g = await gen.generateTemplate(instr({}));
    assert(!g.selfTest.warnings.some(w => /submission address/i.test(w)),
      `the stale warning is still emitted: ${g.selfTest.warnings.join(' | ')}`);
  });
